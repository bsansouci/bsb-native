...
module Belt_ast_attributes
= struct
#1 "belt_ast_attributes.ml"
(* Copyright (C) 2015-2016 Bloomberg Finance L.P.
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Lesser General Public License as published by
...
let generic_to_uncurry_type  kind loc (self : Bs_ast_mapper.mapper) label
    (first_arg : Parsetree.core_type) 
    (typ : Parsetree.core_type)  =
  if label <> "" then
    Bs_syntaxerr.err loc Label_in_uncurried_bs_attribute;

  let rec aux (typ : Parsetree.core_type) = 
    (* in general, 
       we should collect [typ] in [int -> typ] before transformation, 
       however: when attributes [bs] and [bs.this] found in typ, 
       we should stop 
    *)
    let (_, ptyp_attributes) = Ast_attributes.process_attributes_rev typ.ptyp_attributes in
    begin match typ.ptyp_desc with 
      | Ptyp_arrow (label, arg, body)
        -> 
        if label <> "" then
          Bs_syntaxerr.err typ.ptyp_loc Label_in_uncurried_bs_attribute;
        {typ with ptyp_attributes; ptyp_desc = Ptyp_arrow (label, self.typ self arg, aux body)}
      | _ -> self.typ self typ
    end
  in 
  let first_arg = self.typ self first_arg in
  {Parsetree.ptyp_attributes = []; ptyp_loc = loc; ptyp_desc = Parsetree.Ptyp_arrow (label, first_arg, aux typ)}
  (* 
...
let generic_to_uncurry_exp kind loc (self : Bs_ast_mapper.mapper)  pat body 
  = 
  let rec aux (body : Parsetree.expression) = 
    match body.pexp_desc with
    | Pexp_fun (label, whateverthisis, arg, body) -> 
      if label <> "" then
          Bs_syntaxerr.err loc Label_in_uncurried_bs_attribute;
      let (_, pexp_attributes) = Ast_attributes.process_attributes_rev body.pexp_attributes in
      {body with pexp_attributes; pexp_desc = Pexp_fun (label, whateverthisis, self.pat self arg, aux body)}
    | _ -> self.expr self body
  in
  ...
  let () = 
    ...
    | _ -> ()
  in 
  Parsetree.Pexp_fun ("", None, first_arg, aux body)
...
= struct
#1 "belt_ast_core_type_class_type.ml"
...
module Belt_ast_derive_projector
= struct
#1 "belt_ast_derive_projector.ml"
open Ast_helper
...
module Belt_ast_exp_apply
= struct
#1 "belt_ast_exp_apply.ml"
(* Copyright (C) 2018 Authors of BuckleScript
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Lesser General Public License as published by
...
let handle_exp_apply
  ...
  begin match fn.pexp_desc with
    ...
    | Pexp_apply (
      ...
      ->  (* f##paint 1 2 *)
      {e with pexp_desc = Belt_ast_util.method_apply loc self obj name args }
    | Pexp_apply (
        {pexp_desc =
           Pexp_ident  {txt = Lident "#@"  ; loc} ; _},
        [("", obj) ;
      ...
      ->  (* f##paint 1 2 *)
      {e with pexp_desc = Belt_ast_util.property_apply loc self obj name args  } *)
    ...
    | Pexp_ident  {txt = Lident "##" ; loc}
      ...
      begin match args with
        ...
        | [("", obj) ;
          ...
          { e with pexp_desc =
                     Belt_ast_util.js_property loc (self.expr self obj) name
          }
      ...
      begin match args with
        | ["",
          ...
          Exp.constraint_ ~loc
            { e with
              pexp_desc =
                Belt_ast_util.method_apply loc self obj
    ...
    | _ ->
      begin match
          Ext_list.exclude_with_val
            Belt_ast_attributes.is_bs e.pexp_attributes with
      | false, _ -> 
        Bs_ast_mapper.default_mapper.expr self e
      | true, pexp_attributes ->
        {e with 
        (* pexp_desc = Belt_ast_util.uncurry_fn_apply loc self fn args ; *)
                pexp_attributes }
      end
  end

end
module Belt_ast_exp_extension
= struct
#1 "belt_ast_exp_extension.ml"
(* Copyright (C) 2018 Authors of BuckleScript
 * 
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Lesser General Public License as published by
...
let handle_extension record_as_js_object e (self : Bs_ast_mapper.mapper)
  ...
  begin match txt with
    (* 
    | "bs.raw" | "raw" -> 
      Belt_ast_util.handle_raw loc payload
    | "bs.re" | "re" ->
      Exp.constraint_ ~loc
        (Belt_ast_util.handle_raw ~check_js_regex:true loc payload)
        (Ast_comb.to_js_re_type loc) *)
    (* 
    | "bs.external" | "external" ->
      begin match Ast_payload.as_ident payload with 
        | Some {txt = Lident x}
          -> Belt_ast_util.handle_external loc x
    ...
    | "bs.assert" | "assert" ->
      (
        ...
        | PStr [ {pstr_desc = Pstr_eval( e,_)}] -> 
          ...
          let raiseWithString  _locString =      
            (Exp.apply ~loc 
    ...
    (* | "bs.node" | "node" ->
      ...
      begin match Ast_payload.as_ident payload with
        | Some {txt = Lident
          ...
          ->
          let exp =
            Belt_ast_util.handle_external loc (strip name)  in
          let typ =
    ...
    (* | "bs.debugger"|"debugger" ->
      {e with pexp_desc = Belt_ast_util.handle_debugger loc payload}
    | "bs.obj" | "obj" ->
      begin match payload with 
        | PStr [{pstr_desc = Pstr_eval (e,_)}]
          -> 
...
module Belt_ast_derive_abstract
= struct
#1 "belt_ast_derive_abstract.ml"
(* Copyright (C) 2017 Authors of BuckleScript
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Lesser General Public License as published by
...
let () =
  Belt_ast_derive_projector.init ();
  ()
  ...
  { Bs_ast_mapper.default_mapper with
    expr = (fun self ({ pexp_loc = loc } as e) ->
        ...
        | Pexp_extension extension ->
          Belt_ast_exp_extension.handle_extension record_as_js_object e self extension
        | Pexp_apply (fn, args  ) ->
          Belt_ast_exp_apply.handle_exp_apply e self fn args
        | Pexp_fun ("", None, pat , body)
          ->
          begin match Belt_ast_attributes.process_attributes_rev e.pexp_attributes with
            | `Nothing, _
              -> Bs_ast_mapper.default_mapper.expr self e
            |   `Uncurry, pexp_attributes
              ->
              {e with
               pexp_desc = Belt_ast_util.to_uncurry_fn loc self pat body  ;
               pexp_attributes}
            | `Method , _
              ->  Location.raise_errorf ~loc "bs.meth is not supported in function expression"
            | `Meth_callback , pexp_attributes
              ->
              {e with pexp_desc = Belt_ast_util.to_method_callback loc  self pat body ;
                      pexp_attributes }
          end
        (* 
        ...
        | Pexp_function cases ->
          ...
          begin match
              Belt_ast_attributes.process_pexp_fun_attributes_rev e.pexp_attributes
            with
            | `Nothing, _ ->
              Bs_ast_mapper.default_mapper.expr self  e
            | `Exn, pexp_attributes ->
              Belt_ast_util.convertBsErrorFunction loc self  pexp_attributes cases
          end
          ...
          if !record_as_js_object then
            (match opt_exp with
             | None ->
               { e with
                 pexp_desc =
                   Belt_ast_util.record_as_js_object loc self label_exprs;
               }
             | Some e ->
               Location.raise_errorf
        ...
        (* | Pexp_object {pcstr_self;  pcstr_fields} ->
          begin match Belt_ast_attributes.process_bs e.pexp_attributes with
            | `Has, pexp_attributes
              ->
              {e with
               pexp_desc =
                 Belt_ast_util.ocaml_obj_as_js_object
    ...
    typ = (fun self typ ->
        Belt_ast_core_type_class_type.handle_core_type self typ record_as_js_object);
    (* class_type =
      (fun self ({pcty_attributes; pcty_loc} as ctd) ->
         match Belt_ast_attributes.process_bs pcty_attributes with
         | `Nothing,  _ ->
    ...
    signature_item =  begin fun
      ...
      | Psig_type (_ :: _ as tdcls) ->
          Belt_ast_tdcls.handleTdclsInSigi self sigi tdcls
      (* 
      | Psig_value prim
        when Belt_ast_attributes.process_external prim.pval_attributes
        ->
    ...
    structure_item = begin fun self (str : Parsetree.structure_item) ->
      ...
      | Pstr_type (_ :: _ as tdcls ) (* [ {ptype_attributes} as tdcl ] *)->
          Belt_ast_tdcls.handleTdclsInStru self str tdcls
        (* | Pstr_extension ( ({txt = ("bs.raw"| "raw") ; loc}, payload), _attrs)
          ->
          Belt_ast_util.handle_raw_structure loc payload
...
module Belt_bsppx_main
= struct
#1 "belt_bsppx_main.ml"
(* Copyright (C) 2015-2016 Bloomberg Finance L.P.
 * 
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Lesser General Public License as published by
  ...
  try
    ...
    if n > 2 then
      apply_lazy ~source:a.(n - 2) ~target:a.(n - 1)
        !Belt_ppx_entry.rewrite_implementation
        !Belt_ppx_entry.rewrite_signature
    else
      begin
