...
function is_implicit(n) {
  if (is_relative(n) && (n.length < 2 || $$String.sub(n, 0, 2) !== "./") && (n.length < 2 || $$String.sub(n, 0, 2) !== ".\\") && (n.length < 3 || $$String.sub(n, 0, 3) !== "../")) {
    if (n.length < 3) {
      return true;
    } else {
...
function check_suffix(name, suff) {
  if (name.length >= suff.length) {
    ...
    return Caml_string.bytes_to_string(Bytes.lowercase(Caml_string.bytes_of_string(s))) === Caml_string.bytes_to_string(Bytes.lowercase(Caml_string.bytes_of_string(suff)));
  } else {
    return false;
  }
}
...
function dirname(s) {
  ...
  var dir = generic_dirname(is_dir_sep, current_dir_name, match[1]);
  return match[0] + dir;
}

function basename(s) {
  var match = drive_and_path(s);
  return generic_basename(is_dir_sep, current_dir_name, match[1]);
}

var temp_dir_name$2 = temp_dir_name$1;
